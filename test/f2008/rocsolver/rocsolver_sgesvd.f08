!!!!!!!!!!!!!/
! sgesvd example (single-precision singular value decomposition)
...
program sgesvd
  ...
  real(c_float) :: hV(3,3)    ! right singular vectors (stored as V**T)
  real(c_float) :: hE(2)      ! superdiagonal workspace (min(M,N)-1)
  real(c_float) :: recon(3,3), Sd(3,3)

  real(c_float), pointer :: dA(:,:)
  real(c_float), pointer :: dS(:)
  real(c_float), pointer :: dU(:,:)
  real(c_float), pointer :: dV(:,:)
  ...
  !
  write(*,"(a)",advance="no") "-- Running test 'rocsolver_sgesvd' (Fortran 2008 interfaces) - "
