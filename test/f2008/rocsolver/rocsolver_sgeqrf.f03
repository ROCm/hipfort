!!!!!!!!!!!!!/
! sgeqrf example (single-precision QR factorization)
...
program sgeqrf
 ...
 -0.15384615384615385,        5.5555555555555559E-002,  -35.000000000000000/), shape(hResult), order=(/2,1/))
  ...
  real(c_float) :: hIpiv(3) ! CPU buffer for Householder scalars

  real(c_float), pointer :: dA(:,:)   ! GPU buffer for A
  real(c_float), pointer :: dIpiv(:)  ! GPU buffer for Householder scalars

  type(c_ptr) :: handle ! rocblas_handle

  real :: error
  ! single precision: use a relative tolerance (absolute error on
  ! magnitude-~175 entries exceeds 10*epsilon in single precision)
  real, parameter :: error_max = 1.0e-4
  !
  write(*,"(a)",advance="no") "-- Running test 'rocsolver_sgeqrf' (Fortran 2008 interfaces) - "
