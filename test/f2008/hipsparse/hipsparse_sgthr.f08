!!!!!!!!!!!!!!
! hipsparse Sgthr example (gather y[xInd] -> xVal, single)
...
program hipsparse_sgthr
  ...
  real(c_float) :: hY(5) = (/10.0, 20.0, 30.0, 40.0, 50.0/)
  integer(c_int) :: hXind(3) = (/0, 2, 4/)
  real(c_float) :: hXval(3)
  real(c_float) :: hExp(3) = (/10.0, 30.0, 50.0/)
  type(c_ptr) :: handle = c_null_ptr
  real(c_float), pointer :: dY(:), dXval(:)
  integer(c_int), pointer :: dXind(:)
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_sgthr' (Fortran 2008 interfaces) - "
  ...
  call hipsparseCheck(hipsparseSgthr(handle, nnz, dY, dXval, dXind, HIPSPARSE_INDEX_BASE_ZERO))
