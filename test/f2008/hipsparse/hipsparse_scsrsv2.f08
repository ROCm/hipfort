!!!!!!!!!!!!!!
! hipsparse Scsrsv2 example (sparse triangular solve v2, single)
...
program hipsparse_scsrsv2
  ...
  real(c_float) :: hVal(6) = (/2.0, 1.0, 2.0, 3.0, 1.0, 2.0/)
  real(c_float) :: hF(3) = (/2.0, 5.0, 11.0/)
  real(c_float) :: hX(3)
  ...
  real(c_float), pointer :: dVal(:), dF(:), dX(:)
  type(c_ptr) :: dBuf
  integer(c_int) :: bufSize
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_scsrsv2' (Fortran 2008 interfaces) - "
