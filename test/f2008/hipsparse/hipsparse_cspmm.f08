!!!!!!!!!!!!!!
! hipsparse SpMM example (single complex, C = alpha*A*B + beta*C)
...
program hipsparse_cspmm
  ...
  complex(c_float_complex) :: h_B(3,2) = reshape((/ &
    (1.0,0.0), (2.0,1.0), (3.0,0.0), (4.0,1.0), (5.0,0.0), (6.0,1.0)/), (/3,2/))
  complex(c_float_complex) :: h_C(3,2)
  complex(c_float_complex) :: h_Adense(3,3), h_expected(3,2)
  complex(c_float_complex), target :: alpha = (1.0,0.0), beta = (0.0,0.0)

  integer(c_int), pointer :: d_csr_row_ptr(:), d_csr_col_ind(:)
  complex(c_float_complex), pointer :: d_csr_val(:), d_B(:,:), d_C(:,:)
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_cspmm' (Fortran 2008 interfaces) - "

  h_Adense = (0.0,0.0)
  h_Adense(1,1) = (1.0,1.0); h_Adense(1,3) = (2.0,0.0)
  h_Adense(2,2) = (3.0,1.0)
  h_Adense(3,1) = (4.0,0.0); h_Adense(3,3) = (5.0,1.0)
  ...
  call hipsparseCheck(hipsparseCreateDnMat(matC, int(M,c_int64_t), int(Ncol,c_int64_t), int(M,c_int64_t), &
       c_loc(d_C), HIP_C_32F, HIPSPARSE_ORDER_COL))
  ...
  call hipsparseCheck(hipsparseSpMM_bufferSize(handle, HIPSPARSE_OPERATION_NON_TRANSPOSE, &
       ...
       HIP_C_32F, HIPSPARSE_SPMM_ALG_DEFAULT, buffer_size))
  ...
  call hipsparseCheck(hipsparseSpMM(handle, HIPSPARSE_OPERATION_NON_TRANSPOSE, &
       HIPSPARSE_OPERATION_NON_TRANSPOSE, c_loc(alpha), matA, matB, c_loc(beta), matC, &
       HIP_C_32F, HIPSPARSE_SPMM_ALG_DEFAULT, d_buffer))
