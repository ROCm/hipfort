!!!!!!!!!!!!!!
! dsygvdx example (partial generalized symmetric-definite eigensolver, Fortran 2008)
! see: https:!rocm.docs.amd.com/projects/hipSOLVER/en/latest/
!
! sygvdx solves a selected subset of A*x = lambda*B*x (itype=1). range=all requests
...
program hipsolver_dsygvdx
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsolver_dsygvdx' (Fortran 2008 interfaces) - "
  ...
  call hipsolverCheck(hipsolverDsygvdx_bufferSize(handle, HIPSOLVER_EIG_TYPE_1, &
       HIPSOLVER_EIG_MODE_VECTOR, HIPSOLVER_EIG_RANGE_ALL, HIPSOLVER_FILL_MODE_UPPER, N, &
       c_loc(dA(1,1)), lda, c_loc(dB(1,1)), ldb, 0.0d0, 0.0d0, 1, N, nevBuf, c_loc(dW(1)), lwork))
  ...
  call hipsolverCheck(hipsolverDsygvdx(handle, HIPSOLVER_EIG_TYPE_1, HIPSOLVER_EIG_MODE_VECTOR, &
       ...
       0.0d0, 0.0d0, 1, N, hNev, c_loc(dW(1)), dWork, lwork, dInfo(1)))
