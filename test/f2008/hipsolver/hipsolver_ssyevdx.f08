!!!!!!!!!!!!!!
! ssyevdx example (partial symmetric eigensolver, Fortran 2008 interfaces)
! see: https:!rocm.docs.amd.com/projects/hipSOLVER/en/latest/
!
! syevdx computes a selected subset of eigenvalues/eigenvectors. range=all requests
! the full spectrum, so nev returns N and A is overwritten with the eigenvectors.
! Workspace via hipsolverS... bufferSize; devInfo device-backed.
! Self-verifying: A0*v_k = lambda_k*v_k.
!!!!!!!!!!!!!!
!
program hipsolver_ssyevdx
  use iso_c_binding
  use hipfort
  use hipfort_check
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsolver_ssyevdx' (Fortran 2008 interfaces) - "
  ...
  call hipsolverCheck(hipsolverSsyevdx_bufferSize(handle, HIPSOLVER_EIG_MODE_VECTOR, &
  ...
  call hipsolverCheck(hipsolverSsyevdx(handle, HIPSOLVER_EIG_MODE_VECTOR, &
       HIPSOLVER_EIG_RANGE_ALL, HIPSOLVER_FILL_MODE_UPPER, N, c_loc(dA(1,1)), lda, &
       0.0, 0.0, 1, N, c_loc(dNev(1)), c_loc(dW(1)), dWork, lwork, c_loc(dInfo(1))))
