!!!!!!!!!!!!!/
! csptrsv example (single-precision complex sparse triangular solve)
...
program csptrsv
  ...
  complex(c_float_complex) :: h_csr_val(6) = (/ (2.,1.), (1.,0.),(3.,-1.), (4.,1.),(5.,0.),(6.,2.) /)

  complex(c_float_complex) :: h_y(3) = (/ (1.,1.),(2.,-1.),(3.,0.) /)  ! known solution
  complex(c_float_complex) :: h_x(3)          ! rhs = L*y
  complex(c_float_complex) :: h_yout(3)       ! recovered solution
  ...
  complex(c_float_complex), pointer :: d_x(:), d_y(:)
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_csptrsv' (Fortran 2008 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecX, int(M,c_int64_t), c_loc(d_x), rocsparse_datatype_f32_c))
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecY, int(M,c_int64_t), c_loc(d_y), rocsparse_datatype_f32_c))
