!!!!!!!!!!!!!/
! zsptrsm example (double-precision complex sparse triangular solve, multiple rhs)
...
program zsptrsm
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_zsptrsm' (Fortran 2008 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnmat_descr(matB, int(M,c_int64_t), int(nrhs,c_int64_t), int(M,c_int64_t), &
                          c_loc(d_X), rocsparse_datatype_f64_c, rocsparse_order_column))
