!!!!!!!!!!!!!/
! csddmm example (complex sampled dense-dense matrix multiplication)
...
program csddmm
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_csddmm' (Fortran 2008 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnmat_descr(matA, int(M,c_int64_t), int(K,c_int64_t), int(M,c_int64_t), &
                          c_loc(d_A), rocsparse_datatype_f32_c, rocsparse_order_column))
