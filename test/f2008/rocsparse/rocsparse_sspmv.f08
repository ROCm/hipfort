!!!!!!!!!!!!!/
! sspmv example (single-precision sparse-matrix dense-vector multiply, y = alpha*A*x + beta*y)
...
program sspmv
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_sspmv' (Fortran 2008 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecX, int(N,c_int64_t), c_loc(d_x), rocsparse_datatype_f32_r))
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecY, int(M,c_int64_t), c_loc(d_y), rocsparse_datatype_f32_r))
  ...
  call rocsparseCheck(rocsparse_spmv(handle, rocsparse_operation_none, c_loc(alpha), matA, vecX, c_loc(beta), vecY, &
                          rocsparse_datatype_f32_r, rocsparse_spmv_alg_default, &
