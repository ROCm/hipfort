!!!!!!!!!!!!!/
! cspmv example (single-precision complex sparse-matrix dense-vector multiply)
...
program cspmv
  ...
  complex(c_float_complex) :: h_x(3) = (/ (1.,0.),(2.,1.),(3.,-1.) /)
  complex(c_float_complex) :: h_y(3)
  complex(c_float_complex) :: h_expected(3)
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_cspmv' (Fortran 2008 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecX, int(N,c_int64_t), c_loc(d_x), rocsparse_datatype_f32_c))
  ...
  call rocsparseCheck(rocsparse_spmv(handle, rocsparse_operation_none, c_loc(alpha), matA, vecX, c_loc(beta), vecY, &
                          rocsparse_datatype_f32_c, rocsparse_spmv_alg_default, &
