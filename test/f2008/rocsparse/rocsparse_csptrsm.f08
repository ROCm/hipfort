!!!!!!!!!!!!!/
! csptrsm example (single-precision complex sparse triangular solve, multiple rhs)
...
program csptrsm
  ...
  complex(c_float_complex) :: h_X(3,2)        ! rhs = L*Y
  complex(c_float_complex) :: h_C(3,2)        ! recovered solution
  ...
  complex(c_float_complex), pointer :: d_X(:,:), d_C(:,:)
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_csptrsm' (Fortran 2008 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnmat_descr(matB, int(M,c_int64_t), int(nrhs,c_int64_t), int(M,c_int64_t), &
                          c_loc(d_X), rocsparse_datatype_f32_c, rocsparse_order_column))
  call rocsparseCheck(rocsparse_create_dnmat_descr(matC, int(M,c_int64_t), int(nrhs,c_int64_t), int(M,c_int64_t), &
                          c_loc(d_C), rocsparse_datatype_f32_c, rocsparse_order_column))
  ...
  call rocsparseCheck(rocsparse_spsm(handle, rocsparse_operation_none, rocsparse_operation_none, c_loc(alpha), &
                          matL, matB, matC, rocsparse_datatype_f32_c, rocsparse_spsm_alg_default, &
