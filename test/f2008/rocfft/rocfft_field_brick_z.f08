program rocfft_field_brick_z
  ...
  ! Host staging buffer
  complex(8), allocatable, target :: hbuf(:)
  ...
  write(*,"(a)",advance="no") &
    "-- Running test 'rocFFT field/brick 2D z' (Fortran 2008 interfaces) - "
  ...
  lengths(1) = N0
  lengths(2) = N1
  ! f2008: c_loc on the whole rank-1 array (rank_1 wrapper uses c_loc(lengths))
  call rocfftCheck(rocfft_plan_create(gpu_plan, &
    rocfft_placement_notinplace, &
    rocfft_transform_type_complex_forward, &
    rocfft_precision_double, &
    two, c_loc(lengths), one, description))
