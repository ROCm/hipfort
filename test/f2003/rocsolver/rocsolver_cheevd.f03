!!!!!!!!!!!!!/
! cheevd example (single-precision Hermitian eigensolver, Fortran 2003 interfaces)
...
! For Hermitian matrices A is complex (8 bytes) but the eigenvalues D (and
! workspace E) are REAL (4 bytes).
!!!!!!!!!!!!!!/
!
program cheevd
  use iso_c_binding
  ...
  real(c_float), target :: hD(3)                  ! eigenvalues (real)
  real(c_float), target :: hE(3)                  ! workspace (real)
  ...
  !
  write(*,"(a)",advance="no") "-- Running test 'rocsolver_cheevd' (Fortran 2003 interfaces) - "

  hA0 = hA ! keep original for the A*v = lambda*v check

  ! Allocate device-side memory (A complex = 8 bytes, D/E real = 4 bytes)
  call hipCheck(hipMalloc(dA, size_A * 8))
  call hipCheck(hipMalloc(dD, size_D * 4))
  ...
  call hipCheck(hipMemcpy(c_loc(hD(1)), dD, size_D * 4, hipMemcpyDeviceToHost))
