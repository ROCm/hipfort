!!!!!!!!!!!!!/
! dsyevj example (double-precision symmetric Jacobi eigensolver, Fortran 2003 interfaces)
! see: https:!rocm.docs.amd.com/projects/rocSOLVER/en/latest/reference/lapack.html
!
! Self-verifying: with evect=original, A is overwritten with the eigenvectors
! (as columns) and W holds the eigenvalues. We confirm A0*v_k = lambda_k*v_k.
!
! f2003 style: device buffers are type(c_ptr) allocated by byte count; host
! data is moved with hipMemcpy + c_loc. syevj writes residual, n_sweeps and
...
program dsyevj
  ...
  real(c_double), target :: hW(3)      ! eigenvalues
  ...
  !
  write(*,"(a)",advance="no") "-- Running test 'rocsolver_dsyevj' (Fortran 2003 interfaces) - "
  ...
  call hipCheck(hipMalloc(dResidual, 8_c_size_t))
  ...
  call hipCheck(hipMemcpy(c_loc(hW(1)), dW, size_W * 8, hipMemcpyDeviceToHost))
