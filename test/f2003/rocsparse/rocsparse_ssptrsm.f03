!!!!!!!!!!!!!/
! ssptrsm example (single-precision sparse triangular solve with multiple rhs, Fortran 2003 interfaces)
...
program ssptrsm
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_ssptrsm' (Fortran 2003 interfaces) - "
  ...
  call hipCheck(hipMemcpy(d_X,           c_loc(h_X(1,1)),         size_X * 4,  hipMemcpyHostToDevice))
