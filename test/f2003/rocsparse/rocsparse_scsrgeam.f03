!!!!!!!!!!!!!/
! scsrgeam example (single-precision sparse matrix addition, Fortran 2003 interfaces)
...
program scsrgeam
  ...
  real(c_float) :: alpha = 1.0, beta = 1.0
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_scsrgeam' (Fortran 2003 interfaces) - "
  ...
  call hipCheck(hipMemcpy(d_val_A,     c_loc(h_val_A(1)),     size_vA * 4,  hipMemcpyHostToDevice))
  ...
  call hipCheck(hipMemcpy(d_val_B,     c_loc(h_val_B(1)),     size_vB * 4,  hipMemcpyHostToDevice))
