!!!!!!!!!!!!!/
! csptrsm example (single-precision complex sparse triangular solve, multiple rhs, Fortran 2003 interfaces)
...
program csptrsm
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_csptrsm' (Fortran 2003 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnmat_descr(matB, int(M,c_int64_t), int(nrhs,c_int64_t), int(M,c_int64_t), &
                          d_X, rocsparse_datatype_f32_c, rocsparse_order_column))
