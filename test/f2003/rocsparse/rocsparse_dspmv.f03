!!!!!!!!!!!!!/
! dspmv example (double-precision sparse-matrix dense-vector multiply, y = alpha*A*x + beta*y, Fortran 2003 interfaces)
...
program dspmv
  ...
  real(c_double), target :: h_x(3) = (/1, 2, 3/)
  real(c_double), target :: h_y(3)
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_dspmv' (Fortran 2003 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecX, int(N,c_int64_t), d_x, rocsparse_datatype_f64_r))
