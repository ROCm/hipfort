...
program scsrilu0
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_scsrilu0' (Fortran 2003 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_scsrilu0_buffer_size(handle, m, nnz, descr, dVal, dRowPtr, &
       dColInd, info, bufSize))
  call hipCheck(hipMalloc(dBuf, max(bufSize, 1_c_size_t)))
  call rocsparseCheck(rocsparse_scsrilu0_analysis(handle, m, nnz, descr, dVal, dRowPtr, dColInd, &
       info, rocsparse_analysis_policy_reuse, rocsparse_solve_policy_auto, dBuf))
  call rocsparseCheck(rocsparse_scsrilu0(handle, m, nnz, descr, dVal, dRowPtr, dColInd, &
