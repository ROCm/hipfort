!!!!!!!!!!!!!/
! cspmv example (single-precision complex sparse-matrix dense-vector multiply, Fortran 2003 interfaces)
...
program cspmv
  ...
  complex(c_float_complex), target :: h_x(3) = (/ (1.,0.),(2.,1.),(3.,-1.) /)
  ...
  write(*,"(a)",advance="no") "-- Running test 'rocsparse_cspmv' (Fortran 2003 interfaces) - "
  ...
  call rocsparseCheck(rocsparse_create_dnvec_descr(vecX, int(N,c_int64_t), d_x, rocsparse_datatype_f32_c))
  ...
  call hipCheck(hipMemcpy(c_loc(h_y(1)), d_y, size_y * 8, hipMemcpyDeviceToHost))
