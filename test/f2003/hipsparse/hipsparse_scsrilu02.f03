!!!!!!!!!!!!!!
! hipsparse Scsrilu02 example (incomplete LU, single, Fortran 2003)
...
program hipsparse_scsrilu02
  ...
  real(c_float), target :: hVal(7) = (/4.0, 1.0, 1.0, 4.0, 1.0, 1.0, 4.0/)
  real(c_float), target :: hOut(7)
  real(c_float) :: hExp(7)
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_scsrilu02' (Fortran 2003 interfaces) - "
  ...
  hExp(2) = 1.0
  hExp(3) = 1.0/4.0
  hExp(4) = 4.0 - (1.0/4.0)*1.0
  hExp(5) = 1.0
  hExp(6) = 1.0/hExp(4)
  hExp(7) = 4.0 - hExp(6)*1.0
  ...
  call hipsparseCheck(hipsparseScsrilu02_bufferSize(handle, m, nnz, descrA, dVal, dRowPtr, &
       dColInd, info, bufSize))
  call hipCheck(hipMalloc(dBuf, int(max(bufSize,1),c_size_t)))
  call hipsparseCheck(hipsparseScsrilu02_analysis(handle, m, nnz, descrA, dVal, dRowPtr, &
       dColInd, info, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, dBuf))
  call hipsparseCheck(hipsparseScsrilu02(handle, m, nnz, descrA, dVal, dRowPtr, dColInd, &
  ...
  write(*,*) "PASSED!"
end program hipsparse_scsrilu02
