!!!!!!!!!!!!!!
! hipsparse SpMM example (single, C = alpha*A*B + beta*C, Fortran 2003)
...
program hipsparse_sspmm
  ...
  real(c_float), target :: h_B(3,2) = reshape((/1, 2, 3, 4, 5, 6/), (/3,2/))
  real(c_float), target :: h_C(3,2)
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_sspmm' (Fortran 2003 interfaces) - "
  ...
  call hipCheck(hipMalloc(d_B,           int(K*Ncol,c_size_t) * 4))
  call hipCheck(hipMalloc(d_C,           int(M*Ncol,c_size_t) * 4))
  ...
  call hipCheck(hipMemcpy(d_B,           c_loc(h_B(1,1)),         int(K*Ncol,c_size_t) * 4, hipMemcpyHostToDevice))
  ...
  call hipCheck(hipMemcpy(c_loc(h_C(1,1)), d_C, int(M*Ncol,c_size_t) * 4, hipMemcpyDeviceToHost))
