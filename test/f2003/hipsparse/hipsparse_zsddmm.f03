!!!!!!!!!!!!!!
! hipsparse SDDMM example (z, sampled dense-dense matmul, Fortran 2003 interfaces)
...
program hipsparse_zsddmm
  ...
  complex(c_double_complex), target :: h_A(3,3) = reshape((/ &
       (1.,0.),(4.,0.),(7.,0.), &
       (2.,0.),(5.,0.),(8.,0.), &
       (3.,0.),(6.,0.),(10.,0.) /), (/3,3/))
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_zsddmm' (Fortran 2003 interfaces) - "
  ...
  call hipCheck(hipMalloc(d_A,           size_A * 16))
  call hipCheck(hipMalloc(d_B,           size_B * 16))
  ...
  call hipCheck(hipMemcpy(d_A,           c_loc(h_A(1,1)),         size_A * 16, hipMemcpyHostToDevice))
  call hipCheck(hipMemcpy(d_B,           c_loc(h_B(1,1)),         size_B * 16, hipMemcpyHostToDevice))

  call hipsparseCheck(hipsparseCreate(handle))
  call hipsparseCheck(hipsparseCreateDnMat(matA, int(M,c_int64_t), int(K,c_int64_t), int(M,c_int64_t), &
       d_A, HIP_C_64F, HIPSPARSE_ORDER_COL))
  ...
  call hipsparseCheck(hipsparseSDDMM_bufferSize(handle, HIPSPARSE_OPERATION_NON_TRANSPOSE, &
       ...
       HIP_C_64F, HIPSPARSE_SDDMM_ALG_DEFAULT, buffer_size))
  d_buffer = c_null_ptr
  ...
  call hipsparseCheck(hipsparseSDDMM_preprocess(handle, HIPSPARSE_OPERATION_NON_TRANSPOSE, &
       HIPSPARSE_OPERATION_NON_TRANSPOSE, c_loc(alpha), matA, matB, c_loc(beta), matC, &
       HIP_C_64F, HIPSPARSE_SDDMM_ALG_DEFAULT, d_buffer))
