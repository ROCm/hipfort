!!!!!!!!!!!!!!
! hipsparse Ccsrilu02 example (incomplete LU, single complex, Fortran 2003)
...
program hipsparse_ccsrilu02
  ...
  complex(c_float_complex), target :: hVal(7) = (/ &
    (4.0,1.0), (1.0,0.0), (1.0,0.0), (4.0,1.0), (1.0,0.0), (1.0,0.0), (4.0,1.0)/)
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_ccsrilu02' (Fortran 2003 interfaces) - "
  ...
  call hipsparseCheck(hipsparseCcsrilu02_bufferSize(handle, m, nnz, descrA, dVal, dRowPtr, &
       dColInd, info, bufSize))
  call hipCheck(hipMalloc(dBuf, int(max(bufSize,1),c_size_t)))
  call hipsparseCheck(hipsparseCcsrilu02_analysis(handle, m, nnz, descrA, dVal, dRowPtr, &
       dColInd, info, HIPSPARSE_SOLVE_POLICY_NO_LEVEL, dBuf))
  call hipsparseCheck(hipsparseCcsrilu02(handle, m, nnz, descrA, dVal, dRowPtr, dColInd, &
  ...
  write(*,*) "PASSED!"
end program hipsparse_ccsrilu02
