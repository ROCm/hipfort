!!!!!!!!!!!!!!
! hipsparse SDDMM example (d, sampled dense-dense matmul, Fortran 2003 interfaces)
...
program hipsparse_dsddmm
  ...
  write(*,"(a)",advance="no") "-- Running test 'hipsparse_dsddmm' (Fortran 2003 interfaces) - "
  ...
  call hipsparseCheck(hipsparseCreateDnMat(matA, int(M,c_int64_t), int(K,c_int64_t), int(M,c_int64_t), &
       d_A, HIP_R_64F, HIPSPARSE_ORDER_COL))
