...
* **Supported-API tables** (`docs/reference/supported_api/supported_api_<lib>.md`) are
  written by `src/tools/gen_supported_api.py` in the
  [rocm-fortran](https://github.com/ROCm/rocm-fortran) generator repository. They are
  served as native Sphinx pages (not run through Doxygen), so their links do not depend
  on the Doxygen version.

Regenerate both (plus the bindings and the Doxygen denylist) with a single command from
the generator repo:
